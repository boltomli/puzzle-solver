...
class TestSettingsPresets:
    """Tests for settings page provider presets definition."""

    def test_provider_presets_exist(self):
        """Should define presets for OpenAI, Ollama, and DeepSeek."""
        from src.ui.pages.settings import _PROVIDER_PRESETS

        names = [p["name"] for p in _PROVIDER_PRESETS]
        assert "OpenAI" in names
        assert "Ollama" in names
        assert "DeepSeek" in names

    def test_provider_presets_have_urls(self):
        """Each preset should have a non-empty URL."""
        from src.ui.pages.settings import _PROVIDER_PRESETS

        for preset in _PROVIDER_PRESETS:
            assert preset["url"], f"{preset['name']} has no URL"
            assert preset["url"].startswith("http"), f"{preset['name']} URL doesn't start with http"

    def test_openai_preset_url(self):
        """OpenAI preset should have the correct URL."""
        from src.ui.pages.settings import _PROVIDER_PRESETS

        openai_preset = next(p for p in _PROVIDER_PRESETS if p["name"] == "OpenAI")
        assert openai_preset["url"] == "https://api.openai.com/v1"

    def test_ollama_preset_url(self):
        """Ollama preset should have the correct URL."""
        from src.ui.pages.settings import _PROVIDER_PRESETS

        ollama_preset = next(p for p in _PROVIDER_PRESETS if p["name"] == "Ollama")
        assert ollama_preset["url"] == "http://localhost:11434/v1"

    def test_deepseek_preset_url(self):
        """DeepSeek preset should have the correct URL."""
        from src.ui.pages.settings import _PROVIDER_PRESETS

        deepseek_preset = next(p for p in _PROVIDER_PRESETS if p["name"] == "DeepSeek")
        assert deepseek_preset["url"] == "https://api.deepseek.com/v1"
